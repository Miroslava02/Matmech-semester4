...
class ant_colony:
		...
		def _pick_path(self):
			...
			for possible_next_location in self.possible_locations:
				...
				if (distance != 0):
					...
					attractiveness[possible_next_location] = pow(pheromone_amount, self.alpha)*pow(100000000, self.beta)
	...
	def _init_ants(self, start):
		...
		for ant in self.ants:
			ant.__init__(start, list(self.nodes), self.pheromone_map, self._get_distance, self.alpha, self.beta)
	...
	def mainloop(self):
		...
		print('Final distance (Ant algorithm): ', self.shortest_distance)
		return ret
